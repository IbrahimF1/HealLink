...
export function ChatPage({ currentUser, otherUser, introduction, onExit }) {
  ...
  const send = () => {
    ...
    const messagePayload = { text: input.trim(), recipient_id: otherUser.id };
    ...
    >
      ...
      <Card className="bg-white dark:bg-neutral-900 border-2 dark:border-neutral-800 shadow-xl">
              ...
              {messages.length === 0 ? (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-8">
                  ...
                  {!icebreakerUsed && !showIcebreaker && (
                    <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={generateIcebreaker} className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-600 hover:to-purple-600 px-5 py-3 text-white font-semibold shadow-lg hover:shadow-xl transition-all">
                      <Sparkles className="h-5 w-5" /> Generate AI Icebreaker
                    </motion.button>
                  )}

                  {!icebreakerUsed && showIcebreaker && (
                    <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="max-w-md mx-auto rounded-xl bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/20 border-2 border-indigo-200 dark:border-indigo-800 p-5 shadow-lg">
                      <div className="flex items-center justify-center gap-2 mb-4">
                        <div className="flex h-8 w-8 items-center justify-center rounded-full bg-indigo-600 dark:bg-indigo-500"><Sparkles className="h-4 w-4 text-white" /></div>
                        <span className="font-semibold text-indigo-900 dark:text-indigo-200">AI Conversation Starter</span>
                      </div>
                      <p className="text-sm text-indigo-800 dark:text-indigo-300 leading-relaxed mb-4 text-center">{currentIcebreaker}</p>
                      <div className="flex gap-2 justify-center">
                        <button onClick={useIcebreaker} className="rounded-xl bg-indigo-600 hover:bg-indigo-700 px-5 py-2.5 text-sm text-white font-semibold transition-all hover:scale-[1.02] shadow-md">Use This Message</button>
                        <button onClick={generateIcebreaker} className="rounded-xl border-2 border-indigo-300 dark:border-indigo-700 px-5 py-2.5 text-sm text-indigo-700 dark:text-indigo-300 font-semibold hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-all">Try Another</button>
                      </div>
                    </motion.div>
                  )}
                </motion.div>
        ...
        <form className="mt-4 flex gap-2" onSubmit={(e) => { e.preventDefault(); send(); }}>
          <input className={inputClass + " flex-1 focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-400 transition-all"} value={input} onChange={e => setInput(e.target.value)} placeholder="Write a message…" autoComplete="off" />
          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} type="submit" disabled={!input.trim()} className={`inline-flex items-center gap-2 ${primaryBtn} px-6 disabled:opacity-40 disabled:cursor-not-allowed transition-transform`}>
