...
// A simple animated background for the hero section
const AnimatedHeroBackground = () => (
  <Canvas style={{ position: 'absolute', top: 0, left: 0, zIndex: 0 }}>
    <Suspense fallback={null}>
      <ambientLight intensity={0.5} />
      <pointLight position={[10, 10, 10]} />
      <AnimatedSphere color="#a78bfa" position={[-1.5, 1, -2]} />
      <AnimatedSphere color="#4f46e5" position={[1.5, -1, -2]} />
      <MeshDistortMaterial
        attach="material"
        distort={0.5}
        speed={2}
...
const AnimatedSphere = ({ color, position }) => {
  ...
  useFrame(({ clock }) => {
    if (ref.current) {
      ref.current.position.y = Math.sin(clock.getElapsedTime()) + position[1];
    }
  });
  return (
...
// Animation container to fade in elements on scroll
...
export function LandingPage({ navigate }) {
  ...
  function TestimonialsCarousel({ testimonials }) {
    ...
    return (
      <div className="relative max-w-4xl mx-auto">
        <div className="relative rounded-2xl p-8 md:p-12 bg-white dark:bg-neutral-900 border border-gray-200 dark:border-neutral-800 shadow-lg overflow-hidden">
          ...
          >
            ...
            <div className="flex items-center gap-4">
              ...
              />
              <div>
                <div className="font-semibold text-lg text-gray-900 dark:text-neutral-100">{t.name}</div>
                <div className="text-sm text-gray-500 dark:text-neutral-400">{t.role}</div>
              </div>
            </div>
          </motion.div>
        </div>
        <div className="flex items-center justify-center gap-4 mt-8">
          <button onClick={goToPrev} className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-gray-300 dark:border-neutral-700 bg-white dark:bg-neutral-900 text-gray-700 dark:text-neutral-200 hover:bg-gray-50 dark:hover:bg-neutral-800 transition-colors disabled:opacity-50" aria-label="Previous testimonial"> ‹ </button>
          ...
          <button onClick={goToNext} className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-gray-300 dark:border-neutral-700 bg-white dark:bg-neutral-900 text-gray-700 dark:text-neutral-200 hover:bg-gray-50 dark:hover:bg-neutral-800 transition-colors disabled:opacity-50" aria-label="Next testimonial"> › </button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-24">
      {/* Hero Section */}
      <Card ref={heroRef} className="relative overflow-hidden p-8 md:p-12 bg-white/50 dark:bg-neutral-950/50 border-2 dark:border-neutral-800 shadow-xl backdrop-blur-lg">
        <AnimatedHeroBackground />
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ duration: 0.8 }}
          className="relative z-10 mx-auto max-w-6xl grid gap-8 md:grid-cols-2 md:items-center"
        >
          <div className="text-center md:text-left">
            ...
            <div className="mt-8 flex justify-center md:justify-start gap-4">
              ...
              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => navigate('matches')} className="rounded-xl px-6 py-3 border text-gray-800 hover:bg-gray-50 dark:border-neutral-700 dark:text-neutral-100 dark:hover:bg-neutral-800 transition-colors">
                See How It Works
              </motion.button>
            </div>
          </div>
      ...
      <AnimatedSection>
        <Card className="bg-white/70 dark:bg-neutral-900/70">
          <div className="grid gap-6 text-center sm:grid-cols-3">
            <div><div className="text-3xl font-semibold">10k+</div><div className="text-sm text-gray-600 dark:text-neutral-400">Messages exchanged</div></div>
            <div><div className="text-3xl font-semibold">92%</div><div className="text-sm text-gray-600 dark:text-neutral-400">Felt more prepared</div></div>
            <div><div className="text-3xl font-semibold">150+</div><div className="text-sm text-gray-600 dark:text-neutral-400">Procedures covered</div></div>
          </div>
        </Card>
      </AnimatedSection>
      ...
      <AnimatedSection>
        <Card className="bg-white/70 dark:bg-neutral-900/70">
          ...
          <div className="mt-6 grid gap-6 sm:grid-cols-3">
            <div className="flex items-start gap-3"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-300 font-semibold">1</div><div><div className="font-semibold">Share your story</div><div className="text-sm text-gray-600 dark:text-neutral-400">Create a short intro about your procedure, interests, and preferences.</div></div></div>
            <div className="flex items-start gap-3"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-300 font-semibold">2</div><div><div className="font-semibold">Get matched</div><div className="text-sm text-gray-600 dark:text-neutral-400">We suggest a mentor who has been through a similar journey and vibe.</div></div></div>
            <div className="flex items-start gap-3"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300 font-semibold">3</div><div><div className="font-semibold">Chat privately</div><div className="text-sm text-gray-600 dark:text-neutral-400">Ask practical questions, swap tips, and feel supported—one-on-one.</div></div></div>
          </div>
        </Card>
      </AnimatedSection>
      ...
      <AnimatedSection>
        <section className="py-8">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 dark:text-neutral-100 mb-3">What people are saying</h2>
            <p className="text-gray-600 dark:text-neutral-400 max-w-2xl mx-auto">Real experiences from patients, mentors, and caregivers who've used HealLink</p>
          </div>
      ...
      <AnimatedSection>
        <Card className="text-center bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-neutral-900 dark:to-neutral-800">
          ...
          <div className="mt-6">
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => navigate('login')} className={primaryBtn}>
              Get Started
            </motion.button>
          </div>
        </Card>
